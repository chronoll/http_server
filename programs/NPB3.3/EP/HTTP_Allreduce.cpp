...
MYEXTERN_C void HTTP_Allreduce(int YOUR_ID, int allreduce_ID, int numprocs, void *send_hiki_DATA_P, void *recv_hiki_DATA_P, int DATA_LEN, const char *DATA_TYPE, const char *op, int isolation)
{
	MYTimer timer("Allreduce",YOUR_ID); // ID,DATA用タイマー
	...
	*/
	...
	const char* httppath[]={
		"/VC/Allreduce/MPI_Allreduce_ID.php",
		"/VC/Allreduce/MPI_Allreduce_DATA.php",
		"/VC/Allreduce/MPI_Allreduce_recDATA.php",
		"/VC/Allreduce/MPI_Allreduce_FLAG.php"
	};
	...
	/* HTTP プロトコル生成 & サーバに送信 */
	body = "ALLREDUCE_ID=" + std::to_string(allreduce_ID) + "&NUM_PROCS=" + std::to_string(numprocs) + "&YOUR_ID=" + std::to_string(YOUR_ID) + "&ISOLATION=" + std::to_string(isolation);
	...
	filename = std::to_string(isolation) + "_AllreduceID_" + std::to_string(allreduce_ID) + "_" + std::to_string(YOUR_ID) + ".txt";
	...
	std::string op_cpp=op;
	std::string additionalvar=\
	"\r\n--xYzZY\r\nContent-Disposition: form-data; name=var;filename=tmp.txt\r\nContent-Type: text/plain\r\n\r\n"+op_cpp+"\n"+std::to_string(DATA_LEN)+"\n";
	data_length+=(int)additionalvar.length();
	...
	sock.mysend(additionalvar.c_str(),(int)additionalvar.length());
	...
	while (1)
	{
		...
		_RECV_FILE(
			...
			*/
			...
			array_position++;
			assert(array_position<=DATA_LEN);
		)
		timer.stop();
	...
	while (1)
	{
		...
		if (res_ok==true){
			logprintf("MPI_Allreducev completed\n\n");
...
#ifdef MYFORTRAN
extern "C" void http_allreduce_(int *YOUR_ID, int *allreduce_ID, int *numprocs, void *send_hiki_DATA_P, void *recv_hiki_DATA_P, int *DATA_LEN, int *DATA_TYPE, int *op, int *isolation)
{
	const char* type_c=ftype_to_ctype(*DATA_TYPE);
	const char* op_c=fop_to_cop(*op);
	
	HTTP_Allreduce(*YOUR_ID, *allreduce_ID, *numprocs, send_hiki_DATA_P, recv_hiki_DATA_P, *DATA_LEN, type_c, op_c, *isolation);
}
#endif
